...
let private getAttributes noTypeInference cultureInfo (element:XElement) =
  [ for attr in element.Attributes() do
      if attr.Name.Namespace.NamespaceName <> "http://www.w3.org/2000/xmlns/" && attr.Name.ToString() <> "xmlns" then
        yield { Name = attr.Name.ToString()
                Type =
                    if noTypeInference then
                        InferedType.Primitive(typeof<string>, None, false)
                    else
                        getInferedTypeFromString cultureInfo attr.Value None
              } ]

let getInferedTypeFromValue noTypeInference cultureInfo (element:XElement) =
    let value = element.Value
    let typ = getInferedTypeFromString cultureInfo value None
    match typ with
    | InferedType.Primitive(t, _, optional) when (not noTypeInference) && t = typeof<string> && value.TrimStart().StartsWith "{" ->
        try        
            match JsonValue.Parse value with
            | JsonValue.Record _ as json -> 
                let jsonType = json |> JsonInference.inferType noTypeInference cultureInfo element.Name.LocalName
...
let inferGlobalType noTypeInference cultureInfo allowEmptyValues (element:XElement) =
  ...
  let initialTypes =
    ...
    |> Seq.map (fun (name, elements) ->
        ...
        let attributes =
          elements
          |> Seq.map (getAttributes noTypeInference cultureInfo)
        ...
        let bodyType = 
          [ for e in elements do
              if not e.HasElements && not (String.IsNullOrEmpty(e.Value)) then
                yield getInferedTypeFromValue noTypeInference cultureInfo e ]
...
let rec inferLocalType noTypeInference cultureInfo allowEmptyValues (element:XElement) =
  let props = 
    [ // Generate record fields for attributes
      yield! getAttributes noTypeInference cultureInfo element
      
      // If it has children, add collection content
      let children = element.Elements()
      if Seq.length children > 0 then
        let collection = inferCollectionType allowEmptyValues (Seq.map (inferLocalType noTypeInference cultureInfo allowEmptyValues) children)
        yield { Name = ""
                Type = collection } 

      // If it has value, add primitive content
      elif not (String.IsNullOrEmpty element.Value) then
        let primitive = getInferedTypeFromValue noTypeInference cultureInfo element
        yield { Name = ""
                Type = primitive } ]
...
let inferType noTypeInference cultureInfo allowEmptyValues globalInference (element:XElement) =
  if globalInference then inferGlobalType noTypeInference cultureInfo allowEmptyValues element
  else inferLocalType noTypeInference cultureInfo allowEmptyValues element
