...
let rec inferType noTypeInference cultureInfo parentName json =
  ...
  | JsonValue.Array ar -> StructuralInference.inferCollectionType (*allowEmptyValues*)false (Seq.map (inferType noTypeInference cultureInfo (NameUtils.singularize parentName)) ar)
  | JsonValue.Record properties ->
      ...
      let props = 
        [ for propName, value in properties -> 
            let t = inferType noTypeInference cultureInfo propName value
            { Name = propName
              Type = t } ]
